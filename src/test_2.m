...
% plot data
times_matlab = zeros(size(vec_nel));
times_custom = zeros(size(vec_nel));
...
xlabel("Number of entries to add, k")
ylabel(sprintf("Averaged elapsed time of %i runs [ms]", n_reps))
title(sprintf("Time taken to add k entries to a %i-by-%i matrix", n, n));
plot(vec_nel, times_matlab, 'DisplayName', 'MATLAB Sparse Matrix');
plot(vec_nel, times_custom, 'DisplayName', 'Custom Sparse Matrix Builder');
